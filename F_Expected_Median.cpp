...
void solve()
{
    ...
    REP(i, 0, (k + 1) / 2)
    {
        ...
        if (reqz <= tz && reqo <= to)
        {
            int w1 = (fact[tz] * inversemod(fact[tz - reqz], MOD)) % MOD;
            w1 = (w1 * inversemod(fact[reqz], MOD)) % MOD;
            int w2 = (fact[to] * inversemod(fact[to - reqo], MOD)) % MOD;
            w2 = (w2 * inversemod(fact[reqo], MOD)) % MOD;
            // cout << w1 << " " << w2 << endl;
            int w = (w1 * w2) % MOD;
            ans = (ans + w) % MOD;
        }
