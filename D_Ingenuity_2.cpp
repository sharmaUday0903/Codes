...
void solve()
{
    ...
    int cntw = count(s.begin(), s.end(), 'W');
    if ((cntn % 2 != cnts % 2) || (cntw % 2 != cnte % 2) || (n == 2 && s[0] != s[1]))
    {
        cout << "NO\n";
        return;
    }
    if(n==4&&cnts==1&&cntw==1&&cnte==1&&cntn==1)
    {
        string ans="";
        REP(i,0,4)
        {
            if(s[i]=='W'||s[i]=='E')ans+='R';
            else ans+='H';
        }
            cout<<ans<<endl;
        return;
