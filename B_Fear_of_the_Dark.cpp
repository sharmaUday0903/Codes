...
void solve()
{
    ...
    cin >> ax >> ay;
    cin >> bx >> by;
    double ans1=max(hypot(ax,ay),hypot(ax-px,ay-py));
    double ans2=max(hypot(bx,by),hypot(bx-px,by-py));
    double ans3=max({hypot(ax,ay),hypot(bx-ax,by-ay)/2,hypot(bx-px,by-py)});
    double ans4=max({hypot(bx,by),hypot(bx-ax,by-ay)/2,hypot(ax-px,ay-py)});
    cout.precision(10);
